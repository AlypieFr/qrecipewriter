...
/*  Custom functions
/*------------------------------------------------------------------------------------*/
/* Supprimer la version de wrdpress dans le code source des pages */
remove_action('wp_head', 'wp_generator');

/* Ne pas afficher de message explicite en cas de faux mot de passe */
...
if ( function_exists( 'add_image_size' ) ) {
	...
	add_image_size( 'post-image',  315, 236, true );
...
// shortcode photos
function photo($atts){
extract(shortcode_atts(array('src' => '', 'titre' => '', 'w' => '', 'h' => ''), $atts));
if ($w=='' && $h==''):
$image = "<a title='$titre' href='http://www.conseilsencuisine.fr/Photos/$src' rel='lightbox[$src]'><img class='alignleft' src='http://www.conseilsencuisine.fr/Photos/$src' alt='Photo' width='250' height='188'/></a>";
else :
$image = "<a title='$titre' href='http://www.conseilsencuisine.fr/Photos/$src' rel='lightbox[$src]'><img class='alignleft' src='http://www.conseilsencuisine.fr/Photos/$src' alt='Photo' width='$w' height='$h'/></a>";
endif ;
return $image;
}
...
//nrelate
function related_woo_fix($inject) {
if ( is_front_page() ) {
return true;
}
return $inject;
}
add_filter('nrelate_related_should_inject', 'related_woo_fix'); // only use for related plugin
?>
